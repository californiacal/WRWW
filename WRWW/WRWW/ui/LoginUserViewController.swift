...
class LoginUserViewController : UIViewController, TTTAttributedLabelDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        signupLabel?.addLinkToURL(NSURL(string:"SIGN UP"), withRange: signupLabelRange)
    ...
    func attributedLabel(label: TTTAttributedLabel!, didSelectLinkWithURL url: NSURL!) {
        ...
        if url.absoluteString == "SIGN UP"
        {
            print("SIGNUP link pressed")
            
            // FIXME: Transition to a login screen instead of an account creation screen
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let mainVC: UIViewController! = storyboard.instantiateViewControllerWithIdentifier("CreateUserUserViewController")
    ...
    @IBAction func onSignupPressed(sender: AnyObject) {
           ...
           passwordTextField.text?.characters.count >= 6
        {
            ...
            AppDelegate.userAccountRepository.loginWithEmail(emailTextField.text, password: passwordTextField.text, success: { (accessToken:LBAccessToken!) in
                print("Success")
                print(accessToken)
